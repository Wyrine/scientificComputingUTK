...
def F(x):
    ...
    # Defining the system of equations for the function
    # F(x) = R - C / (1 + e * sin(theta + alpha))
...
# minTheta occurs when sin(minTheta + alpha) = 1
# therefore minTheta = pi/2 - alpha in radians
# and to convert to degrees mintheta *= 180/pi
minTheta = (180/pi) * (pi/2.0 - x[2])
# minR = C / (1 + e * sin(pi/2)) = C / (1 + e)
minR = x[0]/(1+ x[1])
